...
inline static std::string Join(const std::vector<T>& strs, const char* delimiter) {
  return Falcata::Common::Join(strs, delimiter, true);
}

template<typename T>
inline static std::string Join(const std::vector<T>& strs, size_t start, size_t end, const char* delimiter) {
  return Falcata::Common::Join(strs, start, end, delimiter, true);
}

inline static const char* Atof(const char* p, double* out) {
  return Falcata::Common::Atof(p, out);
}

template<typename T, bool is_float>
struct __StringToTHelperFast {
  const char* operator()(const char*p, T* out) const {
    return Falcata::Common::Atoi(p, out);
  }
};
...
*          Both versions are kept to maintain bit-for-bit the "legacy" Falcata behaviour in terms of precision.
...
struct __StringToTHelper {
  T operator()(const std::string& str) const {
    T ret = 0;
    Falcata::Common::Atoi(str.c_str(), &ret);
    return ret;
  }
};
...
*          Both versions were kept to maintain bit-for-bit the "legacy" Falcata behaviour in terms of precision.
*/
template<typename T>
inline static std::vector<T> StringToArrayFast(const std::string& str, int n) {
  if (n == 0) {
...
inline static std::vector<T> StringToArray(const std::string& str, int n) {
  ...
  std::vector<std::string> strs = Falcata::Common::Split(str.c_str(), ' ');
  CHECK_EQ(strs.size(), static_cast<size_t>(n));
  std::vector<T> ret;
  ret.reserve(strs.size());
  __StringToTHelper<T, std::is_floating_point<T>::value> helper;
...
template<typename T>
inline static std::vector<T> StringToArray(const std::string& str, char delimiter) {
  std::vector<std::string> strs = Falcata::Common::Split(str.c_str(), delimiter);
  std::vector<T> ret;
  ret.reserve(strs.size());
  __StringToTHelper<T, std::is_floating_point<T>::value> helper;
  for (const auto& s : strs) {
