...
class Dataset {
 public:
  ...
  FALCATA_EXPORT Dataset();

  FALCATA_EXPORT Dataset(data_size_t num_data);
  ...
  */
  FALCATA_EXPORT void InitByReference(data_size_t num_data, const Dataset* reference) {
    metadata_.InitByReference(num_data, &reference->metadata());
  }

  FALCATA_EXPORT void InitStreaming(data_size_t num_data,
                                     int32_t has_weights,
  ...
  FALCATA_EXPORT bool CheckAlign(const Dataset& other) const {
  ...
  FALCATA_EXPORT void FinishLoad();
  ...
  FALCATA_EXPORT bool SetFloatField(const char* field_name, const float* field_data, data_size_t num_element);

  FALCATA_EXPORT bool SetDoubleField(const char* field_name, const double* field_data, data_size_t num_element);

  FALCATA_EXPORT bool SetIntField(const char* field_name, const int* field_data, data_size_t num_element);

  FALCATA_EXPORT bool GetFloatField(const char* field_name, data_size_t* out_len, const float** out_ptr);

  FALCATA_EXPORT bool GetDoubleField(const char* field_name, data_size_t* out_len, const double** out_ptr);

  FALCATA_EXPORT bool GetIntField(const char* field_name, data_size_t* out_len, const int** out_ptr);

  /*!
  * \brief Save current dataset into binary file, will save to "filename.bin"
  */
  FALCATA_EXPORT void SaveBinaryFile(const char* bin_filename);

  /*!
   * \brief Serialize the overall Dataset definition/schema to a binary buffer (i.e., without data)
   */
  FALCATA_EXPORT void SerializeReference(ByteBuffer* out);

  FALCATA_EXPORT void DumpTextFile(const char* text_filename);

  FALCATA_EXPORT void CopyFeatureMapperFrom(const Dataset* dataset);

  FALCATA_EXPORT void CreateValid(const Dataset* dataset);
