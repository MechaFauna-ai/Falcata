...
struct Config {
  ...
  */
  ...
  // desc = custom objective function (gradients and hessians not computed directly by Falcata)
  // descl2 = ``custom``
  ...
  // descl2 = **Note**: internally, Falcata uses ``gbdt`` mode for the first ``1 / learning_rate`` iterations
  ...
  // desc = path of training data, Falcata will train from this data
  // desc = **Note**: can be used only in CLI version
  std::string data = "";

  // alias = test, valid_data, valid_data_file, test_data, test_data_file, valid_filenames
  // default = ""
  // desc = path(s) of validation/test data, Falcata will output metrics for these data
  ...
  // desc = **Note**: internally, Falcata constructs ``num_class * num_iterations`` trees for multi-class classification problems
  ...
  // desc = **Note**: when both ``force_col_wise`` and ``force_row_wise`` are ``false``, Falcata will firstly try them both, and then use the faster one. To remove the overhead of testing set the faster one to ``true`` manually
  ...
  // desc = ``0`` means disable bagging; ``k`` means perform bagging at every ``k`` iteration. Every ``k``-th iteration, Falcata will randomly select ``bagging_fraction * 100%`` of the data to use for the next ``k`` iterations
  ...
  // check = >0.0
  // check = <=1.0
  // desc = Falcata will randomly select a subset of features on each iteration (tree) if ``feature_fraction`` is smaller than ``1.0``. For example, if you set it to ``0.8``, Falcata will select 80% of features before training each tree
  // desc = can be used to speed up training
  // desc = can be used to deal with over-fitting
  ...
  // check = >0.0
  // check = <=1.0
  // desc = Falcata will randomly select a subset of features on each tree node if ``feature_fraction_bynode`` is smaller than ``1.0``. For example, if you set it to ``0.8``, Falcata will select 80% of features at each tree node
  // desc = can be used to deal with over-fitting
  ...
  // desc = if set to ``true``, when evaluating node splits Falcata will check only one randomly-chosen threshold for each feature
  // desc = can be used to speed up training
  // desc = can be used to deal with over-fitting
  ...
  // desc = Falcata allows you to provide multiple evaluation metrics. Set this to ``true``, if you want to use only the first metric for early stopping
  ...
  // check = >0
  // desc = used for the categorical features
  // desc = limit number of split points considered for categorical features. See `the documentation on how Falcata finds optimal splits for categorical features <./Features.rst#optimal-split-for-categorical-features>`_ for more details
  // desc = can be used to speed up training
  ...
  // desc = each subset draws its size uniformly from ``[1, min(max_cat_threshold, (number of candidate categories + 1) / 2)]`` and then draws that many categories without replacement; the subset with the best split gain wins, under the same ``min_data_in_leaf`` / ``min_sum_hessian_in_leaf`` / ``min_data_per_group`` / ``max_cat_threshold`` constraints as the exact search
  ...
  // alias = verbose
  // desc = controls the level of Falcata's verbosity
  ...
  // desc = **Note**: can be used only in CLI version
  std::string output_model = "Falcata_model.txt";
  ...
  // desc = Falcata will auto compress memory according to ``max_bin``. For example, Falcata will use ``uint8_t`` for feature value if ``max_bin=255``
  int max_bin = 255;
  ...
  // desc = by default, Falcata will map data file to memory and load features from memory. This will provide faster data loading speed, but may cause run out of memory error when the data file is very big
  ...
  // desc = **Note**: despite the fact that specified columns will be completely ignored during the training, they still should have a valid format allowing Falcata to load file successfully
  ...
  // desc = if ``true``, Falcata will save the dataset (including validation data) to a binary file. This speed ups the data loading for the next time
  ...
  // [no-save]
  // desc = used only in ``prediction`` task
  // desc = control whether or not Falcata raises an error when you try to predict on data with a different number of features than the training data
  ...
  // desc = if ``true``, Falcata will attempt to predict on whatever data you provide. This is dangerous because you might get incorrect predictions, but you could use it in situations where it is difficult or expensive to generate some features and you are very confident that they were never chosen for splits in the model
  ...
  // desc = **Note**: can be used only in CLI version
  std::string output_result = "Falcata_predict_result.txt";
  ...
  FALCATA_EXPORT void Set(const std::unordered_map<std::string, std::string>& params);
