...
X = rng.standard_normal((5000, 20)); y = X @ rng.standard_normal(20) + 0.3*rng.standard_normal(5000)
...
bst.save_model(txt); bst.save_model(falb)
st, sf = os.path.getsize(txt), os.path.getsize(falb)
chk("save_model auto-detects .falb", open(falb,"rb").read(4) == b"FALB", f"({st:,}B txt vs {sf:,}B falb = {st/sf:.1f}x)")
...
lying = os.path.join(d,"lying.txt"); open(lying,"wb").write(open(falb,"rb").read())
...
flc.set_pickle_format("text"); pk_txt = pickle.dumps(bst); flc.set_pickle_format("falb")
chk("pickle FALB smaller than text", len(pk) < len(pk_txt), f"({len(pk):,} vs {len(pk_txt):,} = {len(pk_txt)/len(pk):.1f}x)")
...
out_falb = os.path.join(d,"cli.falb"); out_txt = os.path.join(d,"cli.txt")
r1 = subprocess.run([sys.executable,"-m","falcata","convert",txt,out_falb],capture_output=True,text=True)
r2 = subprocess.run([sys.executable,"-m","falcata","convert",out_falb,out_txt],capture_output=True,text=True)
