...
def case(name, loss, depth=4, iters=25, n=3000, m=6, nan_frac=0.0, cls=False):
    ...
    mdl.fit(X, y)
    ref = mdl.predict(X, prediction_type="RawFormulaVal") if cls else mdl.predict(X)
...
mdl = cb.CatBoostRegressor(
    iterations=20, depth=4, learning_rate=0.3, verbose=0, allow_writing_files=False, random_seed=1
).fit(Xs, ys)
ref = mdl.predict(Xs)
base = np.asarray(from_catboost(mdl).predict(Xs))
d = tempfile.mkdtemp()
jp = os.path.join(d, "m.json")
mdl.save_model(jp, format="json")
chk("from .json path (no catboost needed)", np.allclose(np.asarray(from_catboost(jp).predict(Xs)), ref, atol=1e-9))
chk("from parsed dict", np.allclose(np.asarray(from_catboost(json.load(open(jp))).predict(Xs)), ref, atol=1e-9))
fb = os.path.join(d, "cb.falb")
from_catboost(mdl).save_model(fb)
chk(
    "imported -> FALB -> load",
...
try:
    mc = cb.CatBoostClassifier(
        iterations=8, depth=3, loss_function="MultiClass", verbose=0, allow_writing_files=False
    ).fit(Xs, rng.integers(0, 3, len(Xs)))
    from_catboost(mc)
    chk("refuses multiclass", False)
except ValueError as e:
    chk("refuses multiclass", "multiclass" in str(e).lower(), str(e)[:42])
try:
    Xc = np.c_[Xs, rng.integers(0, 4, len(Xs)).astype(str)]
    cm = cb.CatBoostRegressor(iterations=8, depth=3, verbose=0, allow_writing_files=False, cat_features=[5]).fit(Xc, ys)
    from_catboost(cm)
    chk("refuses categorical features", False)
except ValueError as e:
    chk("refuses categorical features", True, str(e)[:42])
try:
    lg = cb.CatBoostRegressor(iterations=8, depth=4, grow_policy="Lossguide", verbose=0, allow_writing_files=False).fit(
        Xs, ys
    )
    from_catboost(lg)
    chk("refuses non-oblivious trees", False)
except ValueError as e:
