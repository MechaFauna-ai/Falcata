...
bst = xgb.train({"objective": "reg:squarederror", "max_depth": 4, "seed": 1},
                xgb.DMatrix(Xs, label=ys), num_boost_round=15)
ref = bst.predict(xgb.DMatrix(Xs))
base = np.asarray(from_xgboost(bst).predict(Xs))

d = tempfile.mkdtemp()
jp = os.path.join(d, "m.json"); bst.save_model(jp)
chk("from .json path (no xgboost needed)",
    np.allclose(np.asarray(from_xgboost(jp).predict(Xs)), ref, atol=1e-6))
chk("from parsed dict", np.allclose(
    np.asarray(from_xgboost(json.load(open(jp))).predict(Xs)), ref, atol=1e-6))
...
chk("imported -> FALB -> load", np.allclose(
    np.asarray(flc.Booster(model_file=fb).predict(Xs)), base, atol=0),
    f"({os.path.getsize(jp):,}B json vs {os.path.getsize(fb):,}B falb)")

# refusals must be explicit, never a silently wrong model
lin = xgb.train({"booster": "gblinear", "objective": "reg:squarederror"},
                xgb.DMatrix(Xs, label=ys), num_boost_round=3)
try:
    from_xgboost(lin); chk("refuses gblinear", False)
except ValueError as e:
    chk("refuses gblinear", "gbtree" in str(e), str(e)[:44])
Xc = Xs.copy()
dm = xgb.DMatrix(np.c_[Xc, rng.integers(0, 5, len(Xc))].astype(np.float32), label=ys,
                 feature_types=["q"] * 6 + ["c"], enable_categorical=True)
...
try:
    from_xgboost(xgb.train({"objective": "rank:pairwise"}, xgb.DMatrix(Xs, label=(ys > 0).astype(int)),
                           num_boost_round=3))
