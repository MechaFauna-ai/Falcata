...
std::string GBDT::SaveModelToBinary(int start_iteration, int num_iteration,
  ...
  auto add_section = [&sections](uint32_t id, uint32_t dtype, ByteWriter&& w) {
    SectionBuilder sb;
    sb.entry.id = id;
...

  {
    ...
    add_section(kSecMeta, kDTypeBytes, std::move(w));
  }
  {
    ByteWriter w;
    ...
    add_section(kSecTreeIndex, kDTypeBytes, std::move(w));
  }
  {
    ...
    add_section(kSecSplitFeature, feat_dtype, std::move(feat_w));
    add_section(kSecThresholdIdx, thr_idx_dtype, std::move(thr_w));
    add_section(kSecDecisionType, kDTypeI8, std::move(dt_w));
    add_section(kSecLeftChild, child_dtype, std::move(lc_w));
    add_section(kSecRightChild, child_dtype, std::move(rc_w));
    add_section(kSecLeafValue, kDTypeF64, std::move(lv_w));
  }
  {
    ...
    add_section(kSecThresholdDictOffsets, kDTypeU32, std::move(off_w));
    add_section(kSecThresholdDictValues, kDTypeF64, std::move(val_w));
  }
  if (has_cat) {
    ...
    add_section(kSecCatBoundaries, kDTypeU32, std::move(cb_w));
    add_section(kSecCatThreshold, kDTypeU32, std::move(ct_w));
  }
  if (with_stats) {
    ByteWriter w;
    for (int i = start_model; i < num_used_model; ++i) {
    ...
    add_section(kSecStats, kDTypeBytes, std::move(w));
  }
  if (with_diagnostics) {
    ByteWriter w;
    for (int i = start_model; i < num_used_model; ++i) {
    ...
    add_section(kSecDiagnostics, kDTypeBytes, std::move(w));
  }
...
bool GBDT::LoadModelFromBinary(const char* buffer, size_t len) {
  ...
  for (uint32_t i = 0; i < num_sections; ++i) {
    ...
    if (e.codec != kCodecRaw) {
      Log::Fatal("FALB: section %u uses codec %u, unsupported in this build", e.id, e.codec);
    }
  ...
  const SectionEntry& meta = need(kSecMeta);
  std::string meta_text(r.View<char>(meta.offset, meta.stored_len, "metadata"),
                        static_cast<size_t>(meta.stored_len));
  ...
  const SectionEntry& doff = need(kSecThresholdDictOffsets);
  const SectionEntry& dval = need(kSecThresholdDictValues);
  const size_t dict_n_off = static_cast<size_t>(doff.stored_len / sizeof(uint32_t));
  const size_t dict_n_val = static_cast<size_t>(dval.stored_len / sizeof(double));
  const uint32_t* dict_off = r.View<uint32_t>(doff.offset, dict_n_off, "dict offsets");
  const double* dict_val = r.View<double>(dval.offset, dict_n_val, "dict values");

  const SectionEntry& idx = need(kSecTreeIndex);
  r.Require(idx.offset, num_trees, sizeof(TreeIndexRecord), "tree index");
  const TreeIndexRecord* recs =
      r.View<TreeIndexRecord>(idx.offset, num_trees, "tree index");

  const SectionEntry& sf = need(kSecSplitFeature);
  const SectionEntry& ti = need(kSecThresholdIdx);
  const SectionEntry& dt = need(kSecDecisionType);
  ...
  for (uint64_t i = 0; i < num_trees; ++i) {
    ...
    ReadWidened(r, sf.offset + rec.node_offset * elem_size(sf.dtype), n_node,
                sf.dtype, "split_feature", &split_feature);
    ReadWidened(r, lc.offset + rec.node_offset * elem_size(lc.dtype), n_node,
                lc.dtype, "left_child", &left_child);
    ReadWidened(r, rc.offset + rec.node_offset * elem_size(rc.dtype), n_node,
                rc.dtype, "right_child", &right_child);
    {
      const int8_t* p = r.View<int8_t>(dt.offset + rec.node_offset, n_node, "decision_type");
      std::copy(p, p + n_node, decision_type.begin());
    }
    {
      const double* p = r.View<double>(lv.offset + rec.leaf_offset * sizeof(double),
                                       n_leaf, "leaf_value");
      std::copy(p, p + n_leaf, leaf_value.begin());
    }
    {
      std::vector<uint32_t> tidx;
      ReadWidened(r, ti.offset + rec.node_offset * elem_size(ti.dtype), n_node,
    ...
    if (has_cat && rec.num_cat > 0) {
      ...
      const SectionEntry& ct = need(kSecCatThreshold);
      const size_t n_bound = rec.num_cat + 1;
      std::vector<int> cat_boundaries;
      ReadWidened(r, cb.offset + rec.cat_boundary_offset * sizeof(uint32_t), n_bound,
      ...
      const uint32_t* p = r.View<uint32_t>(
          ct.offset + rec.cat_threshold_offset * sizeof(uint32_t), n_thr, "cat_threshold");
      std::copy(p, p + n_thr, cat_threshold.begin());
      TreeIO::SetCategorical(tree.get(), std::move(cat_boundaries), std::move(cat_threshold));
    }

    if (has_stats) {
      const SectionEntry& st = need(kSecStats);
      const uint64_t lw_base = st.offset;
      ...
      {
        const double* p = r.View<double>(lw_base + rec.leaf_offset * sizeof(double),
                                         n_leaf, "leaf_weight");
        std::copy(p, p + n_leaf, leaf_weight.begin());
        const double* q = r.View<double>(iw_base + rec.node_offset * sizeof(double),
                                         n_node, "internal_weight");
        std::copy(q, q + n_node, internal_weight.begin());
      }
      ReadWidened(r, lcnt_base + rec.leaf_offset * sizeof(int32_t), n_leaf,
                  kDTypeI32, "leaf_count", &leaf_count);
      ReadWidened(r, icnt_base + rec.node_offset * sizeof(int32_t), n_node,
    ...
    if (has_diag) {
      const SectionEntry& dg = need(kSecDiagnostics);
      const uint64_t iv_base = dg.offset + total_nodes * sizeof(float);
      std::vector<float> split_gain(n_node);
      std::vector<double> internal_value(n_node);
      const float* p = r.View<float>(dg.offset + rec.node_offset * sizeof(float),
                                     n_node, "split_gain");
      std::copy(p, p + n_node, split_gain.begin());
      const double* q = r.View<double>(iv_base + rec.node_offset * sizeof(double),
