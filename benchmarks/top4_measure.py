"""Top-4 lever measurement battery.

Run when Felix frees the GPU:
...
for dataset, params, rounds in CELLS:
    ...
    for key in KEYS:
        ...
        tag = (
            "OK-identical"
            if md5 == base_md5
            else ("TUNER-OK" if key == "tuner" and md5 == base_md5 else "*** MD5 DIFFERS ***")
        )
    ...
    print(
        f"  {'ALL FOUR':16s} {t:6.1f} t/s ({(t / base_t - 1) * 100:+5.1f}%) "
        f"{'OK-identical' if md5 == base_md5 else '*** MD5 DIFFERS ***'}",
        flush=True,
    )
