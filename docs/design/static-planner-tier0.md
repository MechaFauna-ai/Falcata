...
`EXABOOST_BATCH_CONSTRUCT_FLOOR`
...
only move layout/scheduling, never arithmetic. Every existing `EXABOOST_*` env
...
| 15 | `EXABOOST_BATCH_CONSTRUCT_FLOOR` (device-saturation floor) | default `160` — `cuda_histogram_constructor.hpp:275-281` | **A** | The cautionary tale (§1). Launch-overhead vs tail-idle crossover, distribution-dependent. Tier-1 prior only: "160; do NOT scale from shape — regressed covtype 45% when tried". |
| 16 | `EXABOOST_BATCH_CONSTRUCT_MINROWS` (min rows/thread cap) | default `64` — `cuda_histogram_constructor.hpp:264-270` | **A** | Same family as #15. Perf crossover between per-thread work and parallelism. Prior "64" to tier 1. |
| 17 | `EXABOOST_SMALL_LEAF_ROWS` (direct small-leaf construct threshold) | default `0` (disabled); enable at `8192` — `cuda_histogram_constructor.hpp:315-321` | **A** | Explicitly perf-ambiguous AND arithmetic-sensitive (per-row double adds vs per-block float partials change the fp accumulation; ~1-2% fraud gain, breaks fraud 63/6 exact reproduction — see the comment). Tier 0 must NOT touch this: it is not bit-neutral. Leave OFF by default; tier-1 prior "0". Flagged as a bit-identity hazard. |
...
| 20 | `GHInterleaveEnabled` (float2 g/h interleave) | on iff `sizeof(score_t)==float` — `cuda_histogram_constructor.hpp:288-294` | **P** | Pure type predicate; bit-identical (the comment states values are identical). Already provable at init. Planner confirms; keep `EXABOOST_GH_INTERLEAVE=0`. |
...
**Flow to kernels — env-override-compatible.** Every existing `EXABOOST_*`
...
- knobs that are already `static` env-latched getters (#7 minrows, #15 floor,
  #16, #17, #18, #20, `SupportsBatchedLevel`'s `EXABOOST_BATCH_WIDE`) keep their
...
- `EXABOOST_PLANNER=0` (new, optional) restores every literal fallback wholesale,
...
the current values of #1-#14, #19-#20 and log them under `EXABOOST_HYBRID_DIAG`.
...
- **md5 locks must be UNCHANGED.** The existing per-dataset model md5 locks are
  ...
  suite after each phase, with `EXABOOST_PLANNER=0` (must match baseline) and
