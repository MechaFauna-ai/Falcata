...
def short_cell(c):
    return (c.replace("-quant", "").replace("covtype", "covt")
             .replace("numerai-example", "num-ex").replace("numerai", "num"))
...
ABL, BASE_TIME = load_ablation()

DS_REGS = [("fraud", "deep"), ("covtype", "deep"), ("year", "deep"),
           ("higgs", "deep"), ("epsilon", "deep"), ("airline", "deep"),
           ("numerai", "numerai-deep")]
DS_LABEL = {("numerai", "numerai-deep"): "numerai-deep (30k)",
...
def plot_cross_library():
    ...
    import math
    gm = {c: math.exp(sum(math.log(v[0]) for v in ratios[c].values()) / len(ratios[c]))
          for c in comps}
    fig, ax = plt.subplots(figsize=(11.5, 3.9))
    x = range(len(DS_REGS))
    w = 0.21
    for xi in x:
        ax.bar(xi - 1.5 * w, 1.0, width=w - 0.04, color=LIB_COLOR["falcata-stoch"],
               label="falcata (stochastic)" if xi == 0 else None)
        ax.text(xi - 1.5 * w, 1.0, "1", ha="center", va="bottom", fontsize=7.5,
                color=TEXT)
    for i, c in enumerate(comps):
        ...
        for xi, dr in enumerate(DS_REGS):
            ...
            if dr in ratios[c]:
                v, note = ratios[c][dr]
                ax.bar(xx, v, width=w - 0.04, color=LIB_COLOR[c],
                       label=None if labeled else LIB_LABEL[c])
                labeled = True
                ax.text(xx, v, f"{v:.1f}{note}", ha="center", va="bottom",
                        fontsize=7.5, color=TEXT)
            else:
                ax.text(xx, 1.08, "✗", ha="center", va="bottom", fontsize=9,
                        color=TEXT2)
    ax.set_yscale("log")
    ...
    ax.set_title("GPU training time relative to falcata — all libraries on CUDA, deep regimes",
    ...
    ax.legend(fontsize=8.5, frameon=False, ncols=4, loc="lower left",
...
def plot_quant_modes():
    drs = [("covtype", "deep"), ("year", "deep"), ("higgs", "deep"),
           ("epsilon", "deep"), ("airline", "deep"), ("numerai", "numerai-deep"),
    ...
    for i, lib in enumerate(modes):
        ...
        for xx, v, dr in zip(xs_i, sp, drs):
            if v == 0:
                ax1.text(xx, 0.08, "✗ " + FAIL.get((lib,) + dr, "n/a"), rotation=90,
                         ha="center", va="bottom", fontsize=9, color=TEXT,
                         weight="bold")
    ax1.axhline(1.0, color=TEXT, linewidth=1.3, linestyle="--",
                label="no-quant baseline (1×)")
    ...
    for i, lib in enumerate(modes):
        dq = []
        for ds, reg in drs:
            m = MED.get((lib, ds, reg)); b = MED.get(("falcata-noquant", ds, reg))
            if not (m and b):
                dq.append(None)
                continue
            met = "rmse" if ds == "year" else ("corr_mean" if ds == "numerai" else
                  ("accuracy" if ds == "covtype" else "auc"))
            mv, bv = m["metrics"][met], b["metrics"][met]
            dq.append((bv - mv) / bv * 100 if met == "rmse" else (mv - bv) / bv * 100)
        xs_i = [xx + (i - 0.5) * w for xx in x]
        ax2.bar(xs_i, [v if v is not None else 0 for v in dq], width=w - 0.04,
                color=LIB_COLOR[lib])
    ...
    bar_ends(ax2)
    fig.suptitle("quant_mode effect, measured on the cross-library sweep (deep regimes)",
                 fontsize=10, y=1.03)
    fig.tight_layout()
...
def plot_construct():
    ...
    for y, lib in zip(ys, libs):
        ...
        v = m["construct_s"]
        ax.barh(y, v, height=0.6,
                color=LIB_COLOR["lightgbm" if lib == "lightgbm-ocl" else lib])
        note = {"catboost": "   (host copy only — quantization deferred into fit)",
                "lightgbm-ocl": "   (CPU binning)",
                }.get(lib, "")
        ax.text(v + 0.6, y, f"{v:.0f}s" + note, va="center", fontsize=9, color=TEXT)
    ax.set_yticks(list(ys), [labels.get(l, LIB_LABEL[l]) for l in libs], fontsize=9.5)
    ax.invert_yaxis()
    ax.set_xlim(0, 64)
    ax.set_xlabel("time until training can start (s)")
    ax.set_title("Ingestion of the numerai matrix (6.8M rows × 3555 features, "
                 "~96 GB float32)", fontsize=10)
    ax.grid(axis="y", visible=False)
    fig.tight_layout()
...
def plot_hybrid_ablation():
    feats = [("hybrid", "hybrid level growth"),
    ...
    for ax, (key, title) in zip(axes, feats):
        vals = [(short_cell(c), ABL[c].get(key, 0)) for c in cells if key in ABL[c]]
        vals.sort(key=lambda t: -t[1])
        ys = range(len(vals))
        ax.barh(list(ys), [v for _, v in vals], height=0.62, color=LIB_COLOR["falcata-stoch"])
        for y, (c, v) in zip(ys, vals):
            ax.text(v, y, f" +{v:.0f}%" if v > 0 else f" {v:.0f}%",
        ...
        ax.margins(x=0.22)
        ax.grid(axis="y", visible=False)
    fig.suptitle("Throughput cost of turning the feature OFF (leave-one-out ablation) — larger = feature worth more", fontsize=10, y=1.04)
    fig.tight_layout()
...
def plot_single_feature(key, title, fname):
    all_vals = [(c, short_cell(c), ABL[c].get(key, 0)) for c in ABL if key in ABL[c]]
    ...
    ys = range(len(vals))
    ax.barh(list(ys), [v for _, v in vals], height=0.62,
...
def plot_hybrid_diagram():
    ...
    def draw_tree(ax, order_labels):
        for child in range(1, 15):
            parent = (child - 1) // 2
            ax.plot(*zip(pos[parent], pos[child]), color=GRID, linewidth=1.2,
                    zorder=1)
        for node in range(7):
            x, y = pos[node]
            ax.scatter([x], [y], s=430, color=BLUE, zorder=2)
            ax.text(x, y, order_labels[node], ha="center", va="center",
    ...
    for d, ytop in ((0, 3), (1, 3 - 1.15), (2, 3 - 2.3)):
        axt_r.add_patch(Rectangle((0.25, ytop - 0.42), 9.5, 0.84,
                                  facecolor=BLUE, alpha=0.10, edgecolor="none"))
        axt_r.text(9.85, ytop, f"batch {d + 1}", fontsize=8.5, color=TEXT2,
                   ha="right", va="center")
    ...
    def strip(ax, blocks, label):
        ...
        for w, busy in blocks:
            ax.add_patch(Rectangle((x, 0.3), w, 0.4,
                                   facecolor=BLUE if busy else GRAY,
                                   edgecolor="none"))
            x += w + 0.04
        ax.text(0.2, 0.92, label, fontsize=9, color=TEXT2, va="top")

    strip(axs_l, [(0.22, True), (1.05, False)] * 7,
          "GPU timeline: short kernels, long CPU-latency gaps — GPU mostly idle")
    strip(axs_r, [(1.6, True), (0.28, False)] * 3,
          "GPU timeline: batched kernels back-to-back (the graph loop, §3, then "
          "removes the last gaps)")
    fig.tight_layout()
...
def plot_curves():
    ...
    for ax, ds, reg in ((ax1, "higgs", "deep"), (ax2, "epsilon", "deep")):
        ...
        for (lib, d, r), curve in CURVES.items():
            if d != ds or r != reg or lib in ("lightgbm-quant", "lightgbm-ocl",
                                              "xgboost-lossguide"):
                continue
            if len(curve) < 2:
                continue
            ...
            ax.plot(xs, ys, color=LIB_COLOR[lib], linewidth=2,
                    label=LIB_LABEL[lib])
        ax.set_xscale("log")
...
def plot_model_size():
    ...
    rows = [("model text (upstream format)", 471.6, ""),
    ...
    ax.invert_yaxis()
    ax.set_xlim(0, 660)
    ax.set_xlabel("model file size (MB) — 45k-tree / 3555-feature numerai "
...
def plot_memory():
    ...
    drs = [("higgs", "deep"), ("epsilon", "deep"), ("airline", "deep"),
           ("numerai", "numerai-deep")]
    ...
    for i, lib in enumerate(libs):
        for xi, (ds, reg) in enumerate(drs):
            ...
            xx = xi + (i - 1.5) * w
            if m is None:
                ax.text(xx, 0.4, "✗ OOM", rotation=90, ha="center", va="bottom",
                        fontsize=8, color=TEXT2)
                continue
            v = m["gpu_mb"] / 1024
            ax.bar(xx, v, width=w - 0.03, color=color,
    ...
    ax.set_title("Peak GPU memory, deep regimes  ·  "
                 "* = upstream's OpenCL backend (its CUDA build OOMs)", fontsize=10)
