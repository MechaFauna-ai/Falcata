...
This is a quick start guide for Falcata CLI version.

Follow the `Installation Guide <./Installation-Guide.rst>`__ to install Falcata first.
...
Falcata supports input data files with `CSV`_, `TSV`_ and `LibSVM`_ (zero-based) formats.
...
Falcata can use categorical features directly (without one-hot encoding).
...
Falcata also supports weighted training, it needs an additional `weight data <./Parameters.rst#weight-data>`__.
...
If one parameter appears in both command line and config file, Falcata will use the parameter from the command line.
...
::
    ...
    falcata config=your_config_file other_args ...
...
::
    ...
    falcata config=train.conf num_trees=10

Examples
--------

-  `Binary Classification <https://github.com/MechaFauna-ai/Falcata/tree/master/examples/binary_classification>`__

-  `Regression <https://github.com/MechaFauna-ai/Falcata/tree/master/examples/regression>`__

-  `Lambdarank <https://github.com/MechaFauna-ai/Falcata/tree/master/examples/lambdarank>`__

-  `Distributed Learning <https://github.com/MechaFauna-ai/Falcata/tree/master/examples/parallel_learning>`__
